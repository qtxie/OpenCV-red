...
;d�finition de la taille(largeur, hauteur) de l'image ftmp
...
while [key <> 27] [
   ...
   ; &src: as byte-ptr! src
    ;applique le filtre m�dian pour r�duire le bruit
    cvSmooth &src &src CV_GAUSSIAN 3 3 0.0 0.0
    
    ;si la touche 'espace' code ASCII '32' est appuy�e, on enregistre le template � partir de l'image 'src'
    if key = 32 [
        cvZero &templ
